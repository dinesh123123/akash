...
import Addcart from "./Removecart";
...
const getHomeData = async () => {
  ...
  const options ={
     ...
     const data = JSON.stringify({
      ...
      await axios.get("http://103.104.74.215:3008/website/recent_rentProduct_list",data,options).then(res => {
...
<section class="product__section section--padding color-scheme-3 pt-0">
            <div class="container">
                <div class="section__heading text-center mb-40">
                    <h2 class="section__heading--maintitle">Recently Added Rent Products</h2>
                </div>
                {/* <strong>You'll love to</strong>
                <p>take these home</p> */}
            <div className='carousel_slider' >
        <Slider {...settings}>
        {lists.map((list) => {
                        return(
        <div onClick={(()=>localStorage.setItem("pro_id",list._id ,"userId",list.userId))}  className="swiper-slide swiper-slide-active"  data-swiper-slide-index={0} role="group" aria-label="1 / 6"  style={{width: '288.667px', marginRight: '30px'}} key={list._id}>
          ...
          <div className="product__items--thumbnail">
            ...
            <ul className="product__items--action d-flex justify-content-center">
              <li className="product__items--action__list">
                <Link className="product__items--action__btn" data-open="modal1" onClick={(()=>localStorage.setItem("pro_id",list._id ,"userId",list.userId))} to="/product">
          ...
          <div className="product__items--content text-center">
            ...
            <div className="product__items--price">
             ...
             {/* <Link className="product__items--action__cart--btn primary__btn btn__style3" to="/cart">
               <span className="add__to--cart__text" onClick={(()=>localStorage.setItem( "pro_id",list._id,"userId",list._id))}>Add to Cart <Addcart/></span>
            </Link>  */}
          </div>
        </div>
      </div>
